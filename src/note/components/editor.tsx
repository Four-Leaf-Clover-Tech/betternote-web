import {Editor, EditorState , ContentState  , RichUtils , DraftEditorCommand} from 'draft-js';
...
class NoteEditor extends Component<NoteEditorComponentProps , NoteEditorState>{
  constructor(props:any){
    ...
    this.state = {
      editorState: contentState!==null&& contentState!==undefined?EditorState.createWithContent(contentState):EditorState.createEmpty()
    }
  }

  componentDidUpdate(prevProps: Readonly<NoteEditorComponentProps>): void {
    if(this.props.noteContent!==prevProps.noteContent){
      var newContent = ContentState.createFromText(this.props.noteContent || " ");
      this.setState({
  ...
  onUnderlineClick = () => {
	...
	onBoldClick = () => {
		this.handelChange(RichUtils.toggleInlineStyle(this.state.editorState, "BOLD"));
	};
